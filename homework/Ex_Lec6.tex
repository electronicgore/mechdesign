...
\begin{ex}
	...
	\begin{align}
	p_{i=1,t=1} = \theta_1k_1^*-(k_1^*)^2 - \left[ \frac{1}{4}\theta_1^2-\frac{1}{2}\theta_1-\frac{1}{16} \right] \\
	=\theta_1 \left( \frac{1}{2}\theta_1-\frac{1}{4} \right) - \left(\frac{1}{2}\theta_1-\frac{1}{4} \right)^2- \left[ \frac{1}{4}\theta_1^2-\frac{1}{2}\theta_1-\frac{1}{16} \right] \\
	=\frac{1}{2}\theta_1.
	\end{align}
...
\subsection*{Solution}
	...
	\paragraph{3)}
	The optimal allocation rule solves the problem: 
	\begin{align*}
	k^\star=\arg\max_{k} \quad  &\mathbb{E_{\varepsilon}}VS\left(\theta_{0},\theta_{1}\right) \\
	= \arg\max_{k}  \quad  &\mathbb{E_{\varepsilon}}\delta k\left[\theta_{1}-\frac{1-F_{0}\left(\theta_{0}\right)}{\phi_{0}\left(\theta_{0}\right)}I_{1}\right],
	\end{align*} 
	i.e. it is the allocation $k$ that maximizes the expected virtual surplus. 
	Since this is linearly increasing in $k$ it is optimal to choose $k=1$ if $\mathbb{E_{\varepsilon}}\left[\theta_{1}-\frac{1-F_{0}\left(\theta_{0}\right)}{\phi_{0}\left(\theta_{0}\right)}I_{1}\right]>0$ and otherwise $k=0$. The equivalent expression for the static model is: 
	\begin{gather*}
	k^\star=\arg\max_{k} \quad \mathbb{E_{\theta}}k\left[\theta-\frac{1-F\left(\theta\right)}{\phi\left(\theta\right)}\right].    
	\end{gather*} 
	The optimal allocation in the static setting is then $k=1$ if $\mathbb{E_{\theta}}\left[\theta-\frac{1-F\left(\theta\right)}{\phi\left(\theta\right)}\right]>0$ and $k=0$ otherwise. The main difference between the optimal allocation in the static and dynamic case respectively is then the presence of the impulse response function. The second term is the information rent, which 
	the seller must "leave" for the buyer to provide incentives to real types (i.e. extract information). In a dynamic setting, the information rent might be smaller $(I<1)$ or larger $(I>1)$ than the static setting. The degree of uncertainty will determine this: If the uncertainty of $\theta$ in the static setting is smaller than the uncertainty of $\theta_1$ conditional on $\theta_0$ in the dynamic setting then the impulse response will be larger than one.
