...
\begin{ex}
	...
	The designer's goal is allocating the item efficiently, i.e., to the player with the highest valuation $\theta_i$. The designer does not care about profit and discounts the future using discount factor $\delta \leq 1$.
	
	\begin{enumerate}
		\item (5pts) Suppose the item was not allocated in period $t=1$, i.e., the designer still has the item in possession at the beginning of $t=2$. Propose a (static) mechanism that the designer can use to allocate the item efficiently among players $i=c,d$ at $t=2$. The mechanism must, of course, be incentive compatible, at least in Bayesian sense. Describe your mechanism completely.
		\item (5pts) Move on to $t=1$. The designer faces a choice between giving the item to one of the players $i=a,b$ or keeping it until $t=2$. 
		...
		\item (5pts) Describe the efficient allocation rule $\kappa^* = (k^*_1(\theta_1,\theta_2), k^*_2(\theta_3,\theta_4))$.
		\item (10pts) Propose a full dynamic mechanism that implements the efficient allocation rule $\kappa^*$ in both periods. Describe your mechanism completely.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		The best way to implement this efficient allocation $k^*_2$ is to use the VCG/pivot mechanism $(k^*_2,p^{VCG}_2)$, where the payments are given by
		\begin{align*}
		...
		\end{align*}
		where $i,j \in \{c,d\}$, $j\neq i$, $\mathbb{I} \{\cdot\}$ is the indicator function, and $k^{-i}_2$ is the efficient allocation that ignores player $i$ (i.e., here it prescribes the item must be given to $j$ for all $\theta=(\theta_c,\theta_d)$). As we know from lectures, VCG is DSIC.
		...
		\emph{Grading note}: the student can use off-the-shelf mechanisms (VCG or AGV), in which case full credit is awarded if the student describes both the [efficient] allocation rule $k^*$ and the payment rule $p$ used by the mechanism. Proof of incentive compatibility is not required. NOTE: simply using Groves' transfers and leaving $h_i(\theta_{-i})$ unspecified does not count as ``full description'' of the mechanism and is penalized by 1 point.
		
		If the student does not use one of the aforementioned mechanisms, they should explicitly mention that their mechanism is direct (or specify what players must report to the mechanism) and then specify allocation and payment rules $(k,p)$ used in their mechanism as respective functions of players' reports. They should then prove that their mechanism is incentive compatible. (Allocation rule $k$ must still be the efficient $k^*$.)
		...
		Note that valuations of players $i=c,d$ are not known to anyone at $t=1$, hence ex post efficiency is unattainable, and interim efficiency is the most we can achieve. The expected value from not allocating the item at $t=1$ is given by $\frac{2\delta}{3}$, as computed in part 3. The efficient allocation is thus
		\begin{align*}
...
\begin{ex}
	...
	\begin{equation*}
		u_{b,t} (k,p,\theta_t) = \sum_{s=t}^2 \mathbb{E} \left[ \theta_s k_s - p_s \right],
	\end{equation*}
	...
	The seller chooses a mechanism $\{(k_t,p_t)\}_{t \in \{1,2\}}$ that maximizes the ex ante expected revenue $U_s = \mathbb{E} \left[ p_1 + p_2 \right]$. This mechanism should be incentive compatible and interim individually rational for the buyer.
	
	\begin{enumerate}
		\item (10pts) Assume that $\varepsilon \equiv 0$, i.e., $\theta_2 = \theta_1$, and this is commonly known. Describe the optimal mechanism.
		\item (10pts) Assume that the seller can perfectly observe $\varepsilon$ at the beginning of $t=2$. Describe the optimal allocation.
		\item (5pts) Assume the same as in part 2 plus that $\varepsilon \sim U[-1,1]$. Find the interim (i.e., conditional on $\theta_1$) expected payment by the buyer in the optimal mechanism.
	...
	\begin{enumerate}
		...
		\begin{align*}
			...
			\end{cases}
		\end{align*}
		Note that payments $(p_1,p_2)$ can be arbitrary as long as $p_1(\theta_1) + p_2(\theta_1) = \frac{1}{2} \cdot \mathbb{I} \left\{ \theta_1 \geq \frac{1}{2} \right\}$.
...
single is arbitrary low.
	...
	\subsection*{Solution}
	\begin{enumerate}
		\item[1-2.] The answers to parts 1 and 2 are the same: under either algorithm the matches are $(m_1, w_1)$ and $(m_2, w_2)$, while $m_3$ remains single. 
		...
		\emph{Grading note}: the problem is simple enough to bruteforce through all possible matchings to verify no others are stable, so it is not necessary to invoke the lattice structure to get full credit. If the student solved (1) and/or (2) incorrectly, arriving to different outcomes in the two questions, then partial credit is available for invoking the lattice structure to say that other stable matchings exist.
		...
		\emph{Grading note}: 3 points for asking each player to report all players' heights or some other reporting structure that allows to cross-verify players' reports. 2 more points for specifying the outcome in case of mismatching reports. 
...
\begin{ex}
	...
	\begin{enumerate}
		...
		\emph{Grading note}: a correct (conditional on parts 1-3) graph of $\Pi^*(q)$ without any further details of the optimal experiment is worth 8 points. Note: graph in Figure \ref{fig:profit_wrong} is \textbf{not} considered correct (since the resulting $\Pi^*(q)$ is not concave).
	\end{enumerate}
