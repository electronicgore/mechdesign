...
\begin{ex}[P2: Implementability conditions]
	...
	\begin{align*}
	U_{i}(v'_i|v_{i})=v_iq_i(v'_i,v_{-i})-t_i(v'_i,v_{-i})\\
	U_i(v_{i})=U_i(v_i|v_i)
	\end{align*}
	where $q_i(v'_i,v_{-i})\in[0,1]$ is the probability that $i$ gets the good when the announcements are $(v'_i,v_{-i})$ and $t_i(v'_i,v_{-i})$ are the payments that need to be done by $i$ in that case.
	\begin{lemma}
...
\begin{ex}[P5: Implementation in Auctions]
	...
	\begin{figure}
		\begin{center}
			\includegraphics[scale=0.4,trim=0 150 0 150, clip]{./Ps2b_pic}
		\end{center}
	...
	\begin{figure}
		\begin{center}
			\includegraphics[scale=0.4]{./ps2}
		\end{center}
		\caption{Solution by Ludvig} \label{fig:2}
	\end{figure}
	We will work with Figure \ref{fig:2}. Of course, there will be some things that are specific to the drawing, but the way we will solve the exercise (mainly, by using the result above) should illustrate the general procedure for different figures. Based on the figure, we construct the following allocation:
