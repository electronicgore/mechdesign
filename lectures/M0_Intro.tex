...
\begin{frame}{What is this course? Problems}
	\begin{itemize}
		...
		\only<1-3>{\visible<3>{\centering\includegraphics[width=0.82\linewidth]{pics/M0/communicating}}}
...
\begin{frame}{First taste: Communication problem}
	To get an idea of the kind of problem we'll be looking at, consider an interaction between two players, described as follows:
	\begin{center}
		\begin{tabular}{r|c|c}
			& Principal & Agent
			\\ \hline 
			Role & \begin{tabular}{@{}c@{}}Has \structure{decision rights}\end{tabular} 
			& \begin{tabular}{@{}c@{}}Has decision-relevant \structure{information}\end{tabular}
			\\ \hline \pause 
			\multirow{3}{*}{Examples} 
			& Politician & Advisor
			\\ \pause
		...
		\begin{itemize}
			\item let $\theta \in \Theta$ denote the state of the world, known to the agent
			\item let $a \in A$ denote the principal's final action
			\item suppose the principal asks the agent to send message $m$ about the state
			\item principal cannot verify whether the message/report is truthful or not
		\end{itemize}
	\end{center}
\end{frame}
...
\begin{frame}{First taste: Communication problem}
	\begin{block}{Communication problem}
		Can the principal \emph{elicit} the agent's \alert{private information}?
	\end{block}
	\begin{itemize}
		\item In other words, will the agent report truthfully?
		\item The answer depends on the \structure{nature of the relationship}
		\begin{itemize}
			\item What will the principal \structure{do} with this knowledge?
			\item How does the agent \structure{feel} about that?
...
\begin{frame}{Common interests}
	...
	\begin{itemize}
		...
		\item $a \in \{l,r\}$ is the \structure<1>{action} the principal will take (where they will go)
...
\begin{frame}{Opposed interests}
	...
	\begin{itemize}
		...
		$a \in \{c,n\}$ is the judge's \structure<1>{action} (verdict: convict or not);
...
\begin{frame}{Partially aligned interests}
	\begin{exampleblock}{Example 3: Partially aligned interests}
		A president (principal) is asking an economic advisor (agent) whether to enact a tax reform
		\begin{center}
			\begin{tabular}{r | c | c | c |}
				$(u_P,u_A)$	& $\theta=l$	& $\theta=n$ 	& $\theta=h$
				\\ \hline 
				$a=n$ 		& $4,1$			& $2,2$			& $0,1$
				\\ \hline
				$a=r$		& $2,0$			& $4,1$			& $2,2$
	...
	\begin{itemize}
		\item $\theta\in \{l,n,h\}$ is the state of the economy (need for a reform), equal probabilities
		\item $a \in \{n,r\}$ is the principal's action (whether to reform), wants $a=r$ if $\theta \in \{n,h\}$
		\item the agent knows $\theta$, wants $a=r$ only if $\theta = h$
		\pause \bigskip 
		\item if the principal believes $m=\theta$, then best response is $a(l)=n$, $a(n)=a(h)=r$
		\pause 
		\item the agent is then partially honest: $m(l)=m(n)=l$, $m(h)=h$! 
		\item $a(l)=n$ is still a best response for the principal, even if they understand the deception
	\end{itemize}
\end{frame}
...
\begin{frame}{Takeaways}
	\begin{itemize}
		...
		\begin{itemize}
			\item E.g., another dimension could be a \alert{payment}
		\end{itemize}
	\end{itemize}
\end{frame}
