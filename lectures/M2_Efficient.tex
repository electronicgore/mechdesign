...
\begin{itemize}
	...
	\item \textbf{NOTE}: it is the \structure{default allocation rules} $k^{-i}(\theta_{-i})$ that each textbook defines differently (or replaces with other things). My version is quite robust, but you can use other default rules if they make more sense in a given setting (so long as the rule for $i$ is independent of $\theta_i$)
