...
\begin{itemize}
	\item Optimal $k$ maximizes \alert{virtual surplus} = \structure{social surplus} -- information rents.
	\begin{itemize}
	...
	\item Now in \structure{dynamics}, \structure{social surplus} is $$S(k,\theta) \equiv \sum_{t\geq 0} \delta^t v(k_t(\theta_t),\theta_t).$$ 
...
\begin{align*}
	VS(\theta_0) &= S(k,\theta) - \frac{1-F_0(\theta_0)}{\phi_0(\theta_0)} \frac{\partial S(k,\theta)}{\partial \theta_0}
	\\
...
\lyxframe{Optimal Mechanism}
\begin{itemize}
	\item To find optimal \structure{allocation}, take expectation of $VS(\theta_0)$ over $\{\varepsilon_t\}$ and maximize over $k$.
	\begin{align*}
...
\lyxframe{Classic Model: ``Marriage''}
\begin{itemize}
	\item Set $M = \{m_1, m_2, ..., m_n\}$ of men, set $W = \{w_1, w_2, ..., w_p\}$ of women.
	\item Every man $m_i$ has strict [ordinal] preferences $\succ_{m_i}$ over $W \cup \{m_i\}$; every woman $w_j$ has preferences $\succ_{w_i}$ over $M \cup \{w_j\}$.
	\begin{itemize}
		\item the latter options mean ``staying single''.
	\end{itemize}
	\begin{definition}
		A \structure{matching} is a one-to-one mapping $\structure{\mu}: M \cup W \to M \cup W$ such that:
		\begin{itemize}
			\item $\mu(m) \in W \cup \{m\}$ for any $m \in M$;
			\item $\mu(w) \in M \cup \{w\}$ for any $w \in W$;
			\item $\mu(\mu(x)) = x$ for any $x \in M \cup W$.
		\end{itemize}
	\end{definition}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item many-to-one matching -- firms and workers; firms have preferences over sets of workers rather than individual employess;
		\item peer effects -- as above, but now workers also have preferences over potential colleagues, rather than the firm itself.
		\item See RS ch.2.3 for particular examples.
	\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Very surprisingly, they organize in a very nice \structure{lattice} structure.
		\item All are contained ``between'' $\mu_{MDA}$ and $\mu_{WDA}$ in terms of players' preferences.
		\item Won't cover here, see RS ch.2.3 and ch.3 for more details.
		\item One corollary: if $\mu_{MDA} = \mu_{WDA}$ then this is the unique stable matching.
	\end{itemize}
	\item Plenty of other fun results, e.g.:
	\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item But why do even need a mechanism in this case? Just let the agents match on their own.
	\end{itemize}
