...
\begin{definition}
	A stable matching \alert{$\mu$} is \alert{M\structure{(W)}-optimal} if every man \structure{(woman)} likes [their match in] it at least as well as [in] any other stable matching.
	
	A stable matching \alert{$\mu$} is \alert{M\structure{(W)}-worst} if every man \structure{(woman)} likes it less than any other stable matching.
\end{definition}
\begin{theorem}
	Matching $\mu_{MDA}$ produced by men-proposing DA algorithm is M-optimal and W-worst.
	
	Matching $\mu_{WDA}$ produced by women-proposing DA algorithm is W-optimal and M-worst.
\end{theorem}
\lyxframeend
...
%TODO: mention here mathing with transfers, many-to-one, etc?
...
\begin{itemize}
	...
	\item There are a set $I$ of players and set $X$ of items.
	\begin{itemize}
		\item Player $i$ has preferences $\succ_i$ over $X$;
		%\item Items have no preferences over whom to be allocated to.
		\item Items have \structure{common} preferences $\succ_X$ over players $I$.
	\end{itemize}
	\item The designer knows $\succ_X$, does not know $\succ_i$.
	\item The designer wants to allocate items to players (at most one item per player) so as to create a stable matching.
\end{itemize}
\lyxframeend
...
\begin{itemize}
	...
	\begin{itemize}
		\item Let players pick items in the order of their (players') priority $\succ_X$. First the $\succ_X$-best player $i$ picks his most preferred $x \in X$, then the second player chooses among the remaining $x \in X$ etc.
...
\end{theorem}
\begin{itemize}
	\item X-DA yields the same outcome as I-DA: first all items send their offers to $\succ_X$-best player, who chooses his favorite, then all remaining items send offers to $\succ_X$-second player etc
...
\begin{itemize}
	...
	\item The designer wants to allocate items to players (at most one item per player) so as to create a Pareto-optimal matching.
...
\begin{itemize}
	...
	\begin{itemize}
		\item Pick any player $i_0$. Ask them to point to person $i_1$ who currently holds $i_0$'s favorite item. Analogously, ask $i_1$ to point to $i_2$ etc. When some $i_N$ points to $i_0$, the cycle closes. Conduct the trades: $i_1$ gives their item to $i_0$, who gives their item to $i_N$ etc.
		\item Remove players $\{i_0,...,i_N\}$ and their items from the game. Start the next cycle with the remaining players and items.
		\item Any player is allowed to point to themselves if they already have the most desired item. This would be a one-player cycle.
	\end{itemize}
\end{itemize}
\lyxframeend
