...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Then only buyer's \alert{initial} info $\theta_0$ then matters for IC.
	\end{itemize}
...
\lyxframe{Optimal Mechanism}
\begin{itemize}
	\item To find optimal allocation, take expectation of $VS(\theta_0)$ over $\{\varepsilon_t\}$ and maximize over $k$.
	\item Then find expected (as of $t=0$) payments from the envelope representation of the buyer's expected utility:
	\begin{align*}
	...
	\end{align*}
	\item Usually these payments will have to be spread out over time and future reports to ensure IC at $t > 0$. No good recipe here.
\end{itemize}
\lyxframeend
...
\begin{itemize}
	\item Let $U_t \equiv (1-\delta) \mathbb{E} \left[\sum_{s \geq t} \delta^{s-t} a_t v_t\right]$. Note $U_t \in \left[0,\bar{U}\right]$ for some $\bar{U}$.
...
\begin{itemize}
	...
	\item Principal cannot commit to future contracts.
\end{itemize}
\lyxframeend
...
\end{theorem}
\begin{itemize}
	\item The relationship inevitably slips out of cooperative mode into one of the uncooperative ones:
	\begin{itemize}
