...
%TODO2020: write own math review. Include order of integrations. Or pay A LOT more attention to revenue equivalence derivation, students are really confused with that step. They also do not even try to use integration by parts.

%TODO2020: maybe written elsewhere, but need to rearrange static MD: merge VCG+gVCG, emphasize IR/BB rather than DSIC/BIC
...
\begin{itemize}
	...
	\begin{itemize}
		\item Player wants her action to match the state.
	\end{itemize}
	\item The designer's objective function is $\pi(a,\omega) = \mathbb{I} (a=G)$.
	\begin{itemize}
		\item Designer wants player to choose $a=G$ regardless of state.
	\end{itemize}
\end{itemize}
\lyxframeend
...
\begin{itemize}
	...
	\end{align*}
	\item Let $Q \in \varDelta(\varDelta(\Omega))$ denote the \structure{distribution over posteriors} $\phi_k$ induced by experiment $\mu$ (from the ex ante perspective).
...
\end{itemize}
\begin{definition}
	A direct experiment $(\mu,\varDelta(\Omega))$ is \alert{consistent} if $\mathbb{E}_Q m_k = \phi_0$.
\end{definition}
\lyxframeend
...
\begin{theorem}[Revelation Principle A]
	\begin{itemize}
		...
		\item For any consistent \structure{distribution} $Q \in \varDelta(\varDelta(\Omega))$ there exists \alert{an experiment} $(\mu,M)$ that induces it.
	\end{itemize}
\end{theorem}
\begin{itemize}
...
\begin{theorem}[Kamenica \& Gentzkow]
	The payoff the designer obtains from the optimal experiment is given by $\Pi^*(\phi_0)$, where $\Pi^*$ is the concave closure of $\Pi$.
\end{theorem}
\begin{itemize}
...
\begin{itemize}
	\item If $\Pi^*(\phi_0) > \Pi(\phi_0)$ then you need to find a set of points \structure{$\{\phi_1, ..., \phi_K\}$} such that:
	\begin{itemize}
		...
		\item $\Pi^*(\phi_0) = \sum_k \tau_k \Pi^*(\phi_k)$.
	\end{itemize}
	\item These $\phi_k$ will be the \structure{posteriors(=messages)} in the optimal experiment $Q$.
	\item You can then use all $(\tau_k,\phi_k)$ to derive the \structure{conditional probabilities $p(m_k | \omega)$} of sending each message $m_k$ in each state $\omega$;
	\begin{itemize}
	...
	\begin{column}{0.5\textwidth}
		{\small
			\begin{itemize}
				...
				\item Decompose $\phi_0$ into points such that $\Pi^*(\phi) = \Pi(\phi)$, namely $\phi_1 = 0$ and $\phi_2 = 1/2$
...
\lyxframe{Revelation Principle B}
\begin{itemize}
	...
	\end{itemize}
	\item We will use decision rules as yet another representation of an experiment, along with $(\mu,M)$ -- message distribution, and $Q$ -- distribution of posteriors.
	\begin{itemize}
...
\lyxframe{Conclusions}
\begin{itemize}
	\item We have seen two approaches to mechanism design (equivalent when both are applicable).
	\item Insight from example: 
	\begin{itemize}
		\item make player take undesirable action only if completely confident it is the right one;
		\item make player barely indifferent when taking the desirable action.
	\end{itemize}
