...
\begin{itemize}
	...
	\item These notes follow Roth and Sotomayor, ch.2,4.
\end{itemize}
\lyxframeend
...
\lyxframe{Beyond DA: going formal}
\begin{theorem}[Roth]
...
\end{theorem}
{\small 
The above statements mean that for every mechanism there is some combination of players and preferences where a deviation exists (that was our example). The below is a stronger statement, that applies to any such combination.
}
\begin{theorem}
	Under any mechanism, if more than one stable matching exists, then at least one player can profitably misrepresent their preferences.
\end{theorem}
\lyxframeend


\lyxframe{``Incomplete'' Information...?}
\begin{itemize}
