...
%%% Early slides are based heavily on earlier versions of this course taught by Christian SchÃƒÂ¶tmuller.
...
\begin{itemize}
	...
	\item This lecture mostly follows Bergemann and V{\"a}lim{\"a}ki (2019).
\end{itemize}
\lyxframeend
...
\begin{itemize}
	...
	\begin{itemize}
		\item In quasilinear model, can ask agent to put collateral at $t=0$, to be repaid at a later date -- this would eliminate incentives to quit mechanism after $t=0$ (if such option exists).
	\end{itemize}
...
\begin{itemize}
	\item As usual, we have the \structure{revelation principle} (Sugaya and Wolitzky, 2017).
...
\begin{itemize}
	...
	\item REMARK: \alert{ex post efficiency} is \alert{unattainable} in dynamics -- $k_t$ must be chosen before $\theta_{t+s}$ learned. \structure{Interim efficiency} is the best we can hope for.
\end{itemize}
\lyxframeend
...
\begin{itemize}
	...
	\end{itemize}
	\item Will focus on one-buyer case.
...
\begin{example}
\begin{itemize}
	\item Suppose that buyer only learns $\theta$ \structure{after} signing up for the mechanism, i.e., timing is:
	\begin{enumerate}
...
\begin{itemize}
	\item So we can argue that the designer can extract all buyer's \structure{future} info at no cost.
	\begin{itemize}
	...
	\item We today take the ``pray that it does'' approach and only worry about extracting the buyer's initial type $\theta_0$ -- we are \structure{back} to the \structure{static problem}.
\end{itemize}
\lyxframeend
...
\begin{itemize}
	\item Optimal allocation $\kappa$ maximizes \alert{virtual surplus} = \structure{social surplus} -- information rents.
	\begin{itemize}
	...
	\item Now in \structure{dynamics}, \structure{social surplus} is $$S(\kappa,\theta) \equiv \sum_{t\geq 0} \delta^t v(k_t(\theta_t),\theta_t).$$ 
...
\lyxframe{Optimal Mechanism}
\begin{itemize}
	\item To find optimal \structure{allocation}, take expectation of $VS(\theta_0)$ over $\{\varepsilon_t\}$ and maximize over $\kappa$.
	\begin{align*}
...
\section{Guo and H{\"o}rner (2018)}

\lyxframe{Dynamic Allocation without Money}
\begin{itemize}
...
\section{Li, Matouschek, Powell (2017)}

\lyxframe{Power Dynamics in Organizations}
\begin{itemize}
	\item One principal, one agent.
	\item Time $t=0,1,...$
...
\section{Lecture 8 \\ Matching Models}
...
\lyxframe{Classic Model: Two-Sided Matching (``Marriage'')}
\begin{itemize}
	...
	\item Goal: finding a ``good'' matching given players' preferences.
\end{itemize}
\lyxframeend
...
\lyxframe{Properties of Matchings}
\begin{definition}
...
\lyxframe{Stability}
\begin{itemize}
	\item Stability is effectively an equilibrium concept for matching problems.
	\begin{itemize}
	...
	\end{itemize}
	\item Note that we are not dealing with any private information.
	\begin{itemize}
		\item Contrary to everything until now.
		\item Players' preferences are commonly known -- goal is just to find the stable/``equilibrium'' matching.
...
\lyxframe{Stability and existence}
...
\begin{itemize}
	...
	\end{itemize}
	\item Also, we would want to know how this stable matching looks...
\end{itemize}
\lyxframeend


\lyxframe{How to Find Your Stable Matching}
...
\lyxframe{Deferred Acceptance: Example}
\begin{example}
	\begin{itemize}
		\item Preferences:
		{\footnotesize
...
\lyxframe{Deferred Acceptance and Stability}
\begin{block}{Claim}
...
\begin{proof}
	\begin{itemize}
		...
		\begin{itemize}
			\item So mathing produced by DA is \structure{individually rational}.
		\end{itemize}
...
\lyxframe{So Many Stable Matchings}
\begin{itemize}
...
\lyxframe{Best Matchings Worst Matchings}
\begin{definition}
	\begin{itemize}
...
\lyxframe{Lattice Structure of the Set of Stable Matchings}
\begin{itemize}
...
\lyxframe{Structure of the Set of Stable Matchings}
\begin{itemize}
...
\lyxframe{Good News and Bad News}
...
\lyxframe{DA is not DSIC: Example}
\begin{example}
	\begin{itemize}
...
\lyxframe{Private types: Beyond DA}
\begin{itemize}
...
\lyxframe{Private types: going formal}
\begin{theorem}[Roth]
...
\lyxframe{Private Types! (Version 2)}
\begin{itemize}
...
\lyxframe{Impossibility (version 2)}
\begin{theorem}[Roth]
...
\lyxframe{Stability and Private Types (version 3)}
\begin{itemize}
...
\lyxframe{Private Types: Summary}
\begin{itemize}
...
\lyxframe{Fair Allocations: Shapley-Scarf model}
\begin{itemize}
	\item There are a set $I$ of players and set $X$ of objects.
	\begin{itemize}
		\item Player $i$ has preferences $\succ_i$ over $X$;
...
\lyxframe{Item Allocation in Matching: Conclusion}
\begin{itemize}
...
\lyxframe{Kelso and Crawford (1982)}
\begin{itemize}
...
\lyxframe{KC '82: Modified DA algorithm}
\begin{itemize}
...
\section{Lecture 9 \\ Information Design}

\lyxframe{What is Information Design?}
\begin{itemize}
	...
	\begin{itemize}
		...
		\item can choose the information that players have about their payoffs.
	\end{itemize}
\end{itemize}
\lyxframeend
...
\begin{itemize}
	...
	\end{itemize}
	\item \alert{Designer's} \structure{objective} function is $\pi(a, \omega)$.
	\item Designer chooses an \structure{experiment} $(\mu,M)$ where $\mu: \Theta \to \varDelta(M^N)$.
\end{itemize}
\lyxframeend


\lyxframe{Experiments}
\begin{itemize}
	\item Designer chooses an \structure{experiment} $\mu: \Theta \to \varDelta(M^N)$.
	\item Experiment $\mu$ generates for each player a signal $m_i$ from the state $\omega$.
	\item Examples:
	\begin{itemize}
		\item Perfectly revealing signals: $m_i = \omega$.
		\item Pooling signals: $m_i = 1$ if $\omega \geq 0$ and $m_i = 0$ if $\omega < 0$.
		\item Mixed signals: $m_i = L$ if $\omega = L$ and $m_i = 
		\begin{cases}
			L & \text{ w.p. }1/2; \\ R & \text{ w.p. }1/2
		\end{cases}$
		if $\omega=R$.
		\item Uninformative signals: $m_i \sim F(M)$ where c.d.f. $F$ is independent of $\omega$.
	\end{itemize}
...
\lyxframe{Experiments}
\begin{itemize}
	\item It is absolutely crucial that the designer \alert{does not know $\omega$} when choosing $\mu$.
	\item Also like in mechanism design, the designer publicly commits to $\mu$ -- any change in $\mu$ is observed by all players.
	\item This commitment power allows the designer to commit ex ante to something that is not necessarily optimal ex post:
	\begin{itemize}
		\item e.g., can commit to telling all truth and nothing but truth -- even if some facts may come up that he'd rather conceal;
		\item or vice versa, can commit to giving no hints to players -- even if really want to reveal the state sometimes.
	\end{itemize}
	\item Without commitment -- if designer chooses a message after learning $\omega$ -- we have a communication problem (rather than information design problem).
	\begin{itemize}
...
\lyxframe{Where is Information Design?}
\begin{itemize}
	\item Examples of such problems (from Kamenica's 2019 survey):
	\begin{itemize}
...
Schwarz 2010), 
		...
		\item law enforcement deployment
(Hernandez \& Neeman2017, Lazear 2006, Rabinovich et al. 2015), 
		
		\item censorship (Gehlbach \& Sonin
2014), 
...
\lyxframe{Two interpretations of Information Design}
\begin{itemize}
	...
	\begin{itemize}
		...
		\item School led by Kamenica, most applications take this stand (even when commitment makes no sense in a particular application).
	\end{itemize}
...
\lyxframe{Illustrative example}
\begin{itemize}
	\item Let $\omega \in \{G,B\}$; let $\phi_0 = \mathbb{P} \{ \omega = G \}$ denote the common prior belief (probability that the state is good);
	\item let $N=1$ (one player), $A = \{G,B\}$ (two actions);
	\item the player's utility is $v(a,\omega) = \mathbb{I} (a=\omega)$.
	\begin{itemize}
		\item Player wants his action to match the state.
	\end{itemize}
	\item The designer's objective function is $\pi(a,\omega) = \mathbb{I} (a=G)$.
	\begin{itemize}
		\item Designer wants player to play $a=G$ regardless of state.
	\end{itemize}
\end{itemize}
\lyxframeend


\lyxframe{Example: timing}
\begin{itemize}
	\item To be clear, the timing in this example (as well as in the general model) is as follows:
	\begin{enumerate}
		\item designer chooses $\mu$ and publicly commits to it
		\item state $\omega$ is determined
		\item message $m$ to the player is drawn according to $\mu(m|\omega)$
		\item player observes $m$ and chooses decision $a$
...
\lyxframe{Example: actions}
\begin{itemize}
	\item Start from the end:
	\item Let $\phi$ denote the player's \structure{posterior belief} (after she observes $m$). What action does she choose?
	\item Denote $\hat{a}(\phi) \equiv \arg \max \mathbb{E}_{\phi(\omega)} [v(a,\omega)]$. If there are many optimal actions, choose the best for the designer.
	\item In our example:
	\begin{equation*}
	...
	\end{equation*}
	\begin{itemize}
		\item For the first time ever the tie-breaking rule is fixed -- we are choosing the designer-best action ($G$).
	\end{itemize}
\end{itemize}
\lyxframeend


\lyxframe{Example: posteriors}
\begin{itemize}
	\item The posterior $\phi$ completely defines the designer's objective function: let
	\begin{equation*}
		\Pi (\phi) \equiv \pi(\hat{a}(\phi)) = \begin{cases}
		1 & \text{ if } \phi \geq 1/2;
		\\
		0 & \text{ if } \phi < 1/2.
		\end{cases}
	\end{equation*}
	\item By choosing an experiment $(\mu,M)$ the designer induces some distribution over posteriors $\phi$.
	\begin{itemize}
		\item What if he could choose any distribution?
		\item Would ideally want $\phi \geq 1/2$ always (after any message $m$).
		\item So if $\phi_0 \geq 1/2$ the principal can do nothing (choose experiment that does nothing) and that would be optimal.
		\item But the ideal is unattainable if $\phi_0 < 1/2$ because beliefs must be \alert{consistent}.
	\end{itemize}
	\item \structure{Belief consistency}: $\mathbb{E} \phi = \phi_0$ (Law of iterated expectations).
	
	The expectation is taken from the ex ante perspective.
	\begin{itemize}
		\item \alert{Remark}: always keep track of perspective. In ID you have ex ante expectations, expectations conditional on $m$, expectations conditional on $\omega$...
	\end{itemize}
\end{itemize}
\lyxframeend


\lyxframe{Example: optimal distribution}
\begin{itemize}
	\item Let us try to find the \structure{best distribution} of posteriors $\phi$ such that $\mathbb{E} \phi = \phi_0 < 1/2$.
	\begin{itemize}
		\item It only matters for the designer whether $\phi < 1/2$ or $\phi \geq 1/2$.
		\item So suppose there are two possible posteriors: $\phi_1 < 1/2$ and $\phi_2 \geq 1/2$, which are induced with respective probabilities $\tau_1$ and $\tau_2 = 1 - \tau_1$. 
		\item Consistency pins the probabilities exactly:
		\begin{align} \nonumber
			\tau_1 \phi_1 + (1-\tau_1) \phi_2 = \phi_0
			\\ \label{eq:ID_cons}
			\Leftrightarrow \tau_1 = \frac{\phi_2 - \phi_0}{\phi_2 - \phi_1}
		\end{align}
		(Note that it also implies that $\phi_1 < \phi_0$.)
	\end{itemize}
	\item The designer gets payoff $1$ whenever $\phi_2$ is induced and $0$ in case of $\phi_1$:
	\vspace{-1em}\begin{align*}
		\mathbb{E}_\phi \Pi (\phi) = \tau_1 \cdot 0 + (1-\tau_1) \cdot 1 = 1-\tau_1
	\end{align*}\vspace{-1em}
	\begin{itemize}
		\item So want to choose $(\phi_1,\phi_2)$ so as to minimize $\tau_1$ subject to \eqref{eq:ID_cons}, $\phi_1 \in [0, \phi_0)$, and $\phi_2 \in [1/2, 1]$.
		\item Easiest done by staring at \eqref{eq:ID_cons} and finding $\phi_1 = 0$, $\phi_2 = 1/2$.
	\end{itemize}
\end{itemize}
\lyxframeend


\lyxframe{Example: from distribution to experiment}
\begin{itemize}
	\item So the optimal distribution is: induce posterior $\phi_1 = 0$ with probability $\tau_1 = 1-2\phi_0$ and posterior $\phi_2 = 1/2$ with probability $\tau_2 = 2 \phi_0$.
	\begin{itemize}
		\item This yields utility $2 \phi_0$ to the designer.
	\end{itemize}
	\item Can we implement this distribution \alert{as an experiment} $(\mu,M)$?
	\begin{itemize}
...
\lyxframe{Example: optimal experiment}
\begin{itemize}
	...
	\begin{itemize}
		\item if \structure{state favorable} to designer then disclose it \structure{truthfully};
		\item if \alert{state bad} for designer then try to \alert{obfuscate} it.
		\item Need \alert{commitment} to mix in $\omega=B$: message $m=g$ gives higher payoff, so without commitment the designer would never send $m=b$.
	\end{itemize}
	\item Player:
	\begin{itemize}
		\item is granted full confidence when taking action that is undesirable for designer;
		\item is made barely indifferent when taking action desired by the designer.
	\end{itemize}
\end{itemize}
\lyxframeend
...
\lyxframe{Actions are irrelevant}
\begin{itemize}
	\item Restrict attention to \alert{public experiments}
	\begin{itemize}
		\item so all players always receive same message $m$ and end up with \structure{same posterior} belief $\phi \in \varDelta (\Theta)$.
	...
	\end{itemize}
	\item Define $\Pi (\phi) \equiv \mathbb{E}_{\phi(\omega)} \left[\pi (\hat{a}(\phi), \omega)\right]$.
	\begin{itemize}
		\item Again, posterior $\phi$ completely defines the designer's objective function.
		\item Expectation needed because $\pi$ depends on $\omega$ in general.
...
\lyxframe{Posteriors}
\begin{itemize}
	\item Let $\phi_k = \phi (\omega | m_k)$ denote the player's posterior belief over states upon observing message $m_k$:
	\begin{align*}
		\phi_k (\omega) = \frac{\phi_0(\omega) \mu(m_k|\omega)}{\sum_{\omega' \in \Omega} \phi_0 (\omega') \mu(m_k | \omega')}
	\end{align*}
	\item Let $Q \in \varDelta(\varDelta(\Theta))$ denote the \structure{distribution over posteriors} $\phi_k$ induced by experiment $\mu$ (from the ex ante perspective).
	\item Belief consistency / LIE: $\mathbb{E}_Q \phi_k = \phi_0$
	\begin{itemize}
...
\begin{itemize}
	...
	\begin{definition}[Direct Experiment A]
		A \structure{direct} experiment is $(\mu,M)$ such that $M = \varDelta(\Theta)$.
	\end{definition}
	\item For the player to actually arrive at beliefs prescribed by a direct experiment, the experiment must be consistent:
\end{itemize}
\begin{definition}
	A direct experiment $(\mu,\varDelta(\Theta))$ is \alert{consistent} if $\mathbb{E}_Q m_k = \phi_0$.
\end{definition}
\lyxframeend
...
\begin{theorem}[Revelation Principle A]
	\begin{itemize}
		\item For \structure{any experiment} $(\mu,M)$ there exists an equivalent \alert{credible consistent} experiment $(\mu,\varDelta(\Theta))$.
		
		\item For any consistent \structure{distribution} $Q \in \varDelta(\varDelta(\Theta))$ there exists \alert{an experiment} $(\mu,M)$ that induces it.
	\end{itemize}
\end{theorem}
\begin{itemize}
	\item So instead of maximizing over all possible experiments $(\mu,M)$ we can maximize over all distributions $Q$, which is a slightly easier problem.
	\item This is what we did in the example, but now we know this is a way we can always go.
	\item From this point onwards, I will also call any consistent distribution $Q$ an experiment.
\end{itemize}
\lyxframeend


\lyxframe{Concavification}
\begin{itemize}
	\item So how do we find the optimal experiment $Q$?
	\item Using the following scary object:
	\begin{definition}[Concave closure]
		Function $\Pi^*(\phi)$ is a \alert{concave closure} of function $\Pi(\phi)$ if it is the pointwise smallest concave function among those that satisfy $\Pi^*(\phi) \geq \Pi(\phi)$ for all $\phi$.
	\end{definition}
	\item Equivalent definition:
	\begin{align*}
		\Pi^*(\phi) \equiv \sup \{ z | (\phi,z) \in co(\Pi) \}
	\end{align*}
	where $co(\Pi)$ is the convex hull of the graph of $\Pi$.
	%\item Recall that $\Pi(\phi)$ is the designer's payoff from inducing posterior $\phi$. Call its concavification $\Pi^*(\phi)$ the \structure{value function} of the designer. Why? See next slide.
	\item Equivalent definition: $\Pi^*$ is the shape that the blanket takes when you throw it over $\Pi$.
\end{itemize}
\lyxframeend


\lyxframe{Concavification: example}
\begin{center}
...
\lyxframe{Optimal experiment: payoff}
\begin{theorem}
	The payoff the designer obtains from the optimal experiment is given by $\Pi^*(\phi_0)$, where $\Pi^*$ is the concave closure of $\Pi(\phi)$.
\end{theorem}
\begin{itemize}
	\item If $\Pi^*(\phi_0) = \Pi(\phi_0)$ then trivial (uninformative) experiment is optimal.
\end{itemize}
\lyxframeend


\lyxframe{Optimal experiment: cookbook}
\begin{itemize}
	\item If $\Pi^*(\phi_0) > \Pi(\phi_0)$ then you need to find a set of points \structure{$\{\phi_1, ..., \phi_k\}$} ``closest'' to $\phi_0$ such that:
	\begin{itemize}
		\item $\phi_0 \in co\left(\{\phi_1, ..., \phi_k\}\right)$;
		\item $\Pi^*(\phi_k) = \Pi(\phi_k)$;
	\end{itemize}
	\item these will be the \structure{posteriors(=messages)} in the optimal experiment $Q$. 
	\item You should then be able to find the \structure{ex ante probabilities $\tau_k$} of each posterior from consistency: $\phi_0 = \sum_k \tau_k \phi_k$,
	\begin{itemize}
		\item check also that $\Pi^*(\phi_0) = \sum_k \tau_k \Pi^*(\phi_k)$, otherwise your points are not ``closest'' to $\phi_0$ among those that satisfy the requirements.
	\end{itemize}
	\item and finally use all $(\tau_k,\phi_k)$ to derive the \structure{conditional probabilities $p(m_k | \omega)$} of sending each message $m_k$ in each state $\omega$ (Bayes' rule + feasibility, as we did in the example).
	\item Congratulations, you have just information designed.
\end{itemize}
\lyxframeend


\lyxframe{Back to example}
\begin{columns}
	\begin{column}{0.5\textwidth}
		\begin{center}
			...
			\draw[->] (0,0) -- (0,1.3);% node[below left]{$\Pi^*(\phi)$};
			...
			\matrix [draw, fill=white, below right] at (0.05,1.35) {
				\draw [piline] ++(-0.3,0) -- ++(0.6,0) node[black,right] {$\Pi(\phi)$}; \\
				\draw [line width=0.6mm, violet, opacity=0.7, dashed] ++(-0.3,0) -- ++(0.6,0) node[black,right,opacity=1] {$\Pi^*(\phi)$}; \\
			};
	...
	\begin{column}{0.5\textwidth}
		{\small
			\begin{itemize}
				\item If $\phi_0 \geq 1/2$ then $\Pi^*(\phi_0) = \Pi(\phi_0)$,
...
\begin{columns}
	...
	\begin{column}{0.5\textwidth}
		{\small
			\begin{itemize}
				\item If $\phi_0 < 1/2$ then $\Pi^*(\phi_0) > \Pi(\phi_0)$.
				\item The ``closest'' points such that $\Pi^*(\phi) = \Pi(\phi)$ are $\phi_1 = 0$ and $\phi_2 = 1/2$
				\item which is exactly what we found while solving the example.
			\end{itemize}
		}
	\end{column}
\end{columns}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item you can draw one in three dimensions (three states),
