...
\begin{itemize}
	...
	\item Key feature -- persistence: $\mathbb{P}(v_{t+1} = v_t) = \rho > 1$.
...
\begin{itemize}
	...
	\begin{center}
		\begin{tabular}{| c | c |}
			\hline
			\emph{usual model} 	& \emph{this model} \\ \hline
			allocation $k$ 	& today's allocation $a_t$\\ \hline
			transfer $t$ 	& continuation util $V_{t+1}$\\ \hline
		\end{tabular}
	\end{center}
	\medskip
	\item Opt. mech: if agent does not require funding today, allow to claim more funding in the future. For $v=H$ agent, funding today is more valuable than in the future (since $\mathbb{E} v_{t+s} < H$), for $v=L$ future funding is more valuable $\Rightarrow$ IC.
\end{itemize}
\lyxframeend
