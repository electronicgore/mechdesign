...
\begin{itemize}
	\item \structure{Periods} $t \in \{1,2,...,T\}$; terminal time $T \leq \infty$; all players (incl. designer) have common \structure{discount} factor $\delta$.
...
\begin{itemize}
	...
	\item Reporting strategies given by $\rho_i = \{r_{i,t}\}_{t=1}^T$, where $r_{i,t}: \Theta_i \times H_t \to \Theta_i$ and $H_t$ is the set of public histories $h_t = {k_s,r_{i,s}}_{s < t}$.
	\item A dynamic direct mechanism is $(\kappa,\pi) = \{k_t,p_t\}_{t=1}^T$, where $k_t: \Theta \times H_t \to K_t$ and $p_t: \Theta \times H_t \to \mathbb{R}^N$.
\end{itemize}
\lyxframeend
